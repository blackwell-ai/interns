...
token via the `token-refresh` edge function. API-key providers (apollo, anthropic) return the stored key the same way, so no key ever
...
def connect_api_key(provider: str, key: str, account: str = "", org_shared: bool = False) -> None:
    """Store an API-key connection (apollo, anthropic). The secret
