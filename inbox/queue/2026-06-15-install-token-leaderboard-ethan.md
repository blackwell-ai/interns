---
title: Install the token-leaderboard feeder on Ethan's machine
created: 2026-06-15
created_by: Armaan
assignee: Ethan
priority: normal
...
Get Ethan onto the team token leaderboard. The board ranks how much each
...
`LEADERBOARD_PERSON=Ethan` to `credentials/.env`, run
...
Ethan shows on the board: `select * from leaderboard` returns a row for Ethan,
and `cron.sh --install` reports the daily job is scheduled.
