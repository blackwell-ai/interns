...
| researcher-daily-digest | Weekday external-signal radar: fetch Reddit + HN → per-post items → filter → synthesize a brain digest + inbox tasks (cron via cron.sh) | fetch, extract, llm, inbox | 2026-07-01 (`researcher-daily-digest-20260701T150052Z-ca6373`) |
