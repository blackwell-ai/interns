...
| researcher-daily-digest | Weekday external-signal radar: fetch Reddit + HN → per-post items → filter → synthesize a brain digest + inbox tasks (cron via cron.sh) | fetch, extract, llm, inbox | 2026-07-02 (`researcher-daily-digest-20260702T172653Z-bacf23`) |
| granola-export | Export Granola meeting notes to context/samarjit-granola/ (keychain decrypt + Granola API; document-id suffix on title+date collisions) | (script; node + macOS keychain + granola API) | 2026-07-02 |
| granola-sync | Publish layer over granola-export (front door `/granola`): export new meeting notes, commit, push (auto-rebase --autostash on remote move); idempotent no-op when nothing is new | (script; granola-export + git) | 2026-07-02 |
